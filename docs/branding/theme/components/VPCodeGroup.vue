...
const getAriaControls = (index: number) => `code-tabpanel-${index}`
const getTabId = (index: number) => `code-tab-${index}`
const getTabPanelId = (index: number) => `code-tabpanel-${index}`
const getAriaLabelledby = (index: number) => `code-tab-${index}`
</script>

<template>
  <div class="VPCodeGroup">
